...
void DurationToString(long delta, LPTSTR *outStr)
{
    ...
    *outStr = (TCHAR *)malloc((MAX_DURATION_STR + 1)* sizeof(TCHAR));
    ...
    if (days > 0) {
        _snprintf(*outStr, MAX_DURATION_STR, "(%d d, %s remaining)", days, hoursStr);
    } else if (hours > 0) {
...
static UINT krenew(void *param)
{
    ...
    // @TODO: logic to check for imported tickets and auto-renew/re-import from MSLSA
...
/* TODO
        if ( code != KRB5KDC_ERR_ETYPE_NOSUPP ||
            ...
            Leash_krb5_error(code, "krb5_get_renewed_creds()", 0, &ctx, &ccache);
*/
        goto cleanup;
    }
...
void CLeashView::OnNMCustomdraw(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = CDRF_DODEFAULT;
    ...
    case CDDS_SUBITEM | CDDS_ITEMPREPAINT:
        ...
        if (pElem && pNMLVCD->iSubItem == 0) {
            CListCtrl &list = GetListCtrl();
            CRect drawRect, nextRect;
            if (list.GetSubItemRect(iItem, 0, LVIR_BOUNDS, drawRect)) {
                HTHEME hTheme = OpenThemeData(pNMLVCD->nmcd.hdr.hwndFrom, L"Explorer::TreeView");
                drawRect.right = drawRect.left + (drawRect.bottom - drawRect.top);
                // @TODO: need hot states, too: TVP_HOTGLYPH, HGLPS_OPENED, HGLPS_CLOSED
